...
public class DanielFun {
	...
	public static void main(String[] args) throws TimeoutException {
		...
		try {
			if (command.equals("create")) {
				...
				System.out.printf("login identity:%n%s%n", pair
						.getKeyMaterial());
	...
	private static void destroySecurityGroupKeyPairAndInstance(
		...
		try {
			...
			System.out.printf("%d: %s terminating instance%n", System
		...
		try {
			System.out.printf("%d: %s deleting keypair%n", System
		...
		try {
			System.out.printf("%d: %s deleting group%n", System
					.currentTimeMillis(), name);
	...
	static void createSecurityGroupAndAuthorizePorts(EC2Client client,
			String name) {
		System.out.printf("%d: creating security group: %s%n", System
				.currentTimeMillis(), name);
	...
	static KeyPair createKeyPair(EC2Client client, String name) {
		System.out.printf("%d: creating keypair: %s%n", System
				.currentTimeMillis(), name);
